         ...
         <div class="mt50 commentHtml"><div>
            ...
            <section class="">
                <section class="question-list lh-bj-list pr">
                    <ul class="pr10">
                    <li v-for="(comment,index) in commetnData.items" v-bind:key="index">
                        <aside class="noter-pic">
                ...
                <div class="paging">
                    ...
                    <a
                    :class="{undisable: !commetnData.hasPrevious}"
                    href="#"
                    title="首页"
                    ...
                    href="#"
                    title="前一页"
                    @click.prevent="gotoPage(commetnData.current-1)">&lt;</a>
                    <a
                    v-for="page in commetnData.pages"
                    :key="page"
                    :class="{current: commetnData.current == page, undisable: commetnData.current == page}"
                    :title="'第'+page+'页'"
                    href="#"
                    @click.prevent="gotoPage(page)">{{ page }}</a>
                    <a
                    :class="{undisable: !commetnData.hasNext}"
                    href="#"
                    title="后一页"
                    @click.prevent="gotoPage(commetnData.current+1)">&gt;</a>
                    <a
                    ...
                    href="#"
                    title="末页"
                    @click.prevent="gotoPage(commetnData.pages)">末</a>
                    <div class="clear"/>
                </div>
                <!-- 公共分页 结束 -->
            </div>
    ...
    methods:{
        initComment(){
            comment.getPageList(this.page, this.limit, this.courseId).then(response => {
               this.commetnData = response.data.data
            })
        },
        initCourseInfo(){
